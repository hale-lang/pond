...
- **Upstream memory-safety bugs — one FIXED, one still open (retested 2026-08-03 at v0.13.0):** the locus-created-via-free-fn-return-inside-a-method-frame heap corruption is **FIXED** (the exact `FRICTION.log § pond/ml/neural` repro runs 20/20 clean; segfaulted 5/5 at 400ac68). The shipped workaround patterns (preallocate + `extract_row_into`; drive training loops from `fn main()` frames) STAY until a dedicated removal pass revalidates them — the entry's second, narrower zero-reads manifestation is not yet retested. Still OPEN and now known SEPARABLE: the migrations form-vec-factory-after-a-caught-failure segfault (`FRICTION.log § pond/migrations` entry 14; 5/5 at v0.13.0) — inline construction after a caught Migrator failure remains binding.
